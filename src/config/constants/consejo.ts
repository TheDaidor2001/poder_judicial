// Datos de los miembros del Consejo Superior del Poder Judicial
export const miembrosConsejo = [
    {
        id: "presidente",
        cargo: "Presidente del Consejo Superior del Poder Judicial",
        nombre: "S.E. OBIANG NGUEMA MBASOGO; Presidente de la república, Jefe de Estado y de Gobierno",
        foto: "/images/cspj/obiang.webp",
        descripcion: "Breve descripción de las funciones principales del Presidente del Consejo Superior del Poder Judicial...",
        funciones: [
            "Gobernar",
            "Representar al Poder Judicial",
            "Dirigir la política judicial",
            "Coordinar con otros órganos del Estado"
        ],
        trayectoria: "Trayectoria profesional del presidente..."
    },
    {
        id: "vicepresidente",
        cargo: "Vicepresidente del Consejo Superior del Poder Judicial",
        nombre: "Excmo. Sr. Don Francisco Evuy Nguema Mikue",
        foto: "/images/cspj/francisco.webp",
        descripcion: "Breve descripción de las funciones principales del Vicepresidente del Consejo Superior del Poder Judicial...",
        funciones: [
            "Suplir al Presidente en sus ausencias",
            "Coordinar comisiones internas",
            "Supervisar áreas específicas delegadas"
        ],
        trayectoria: "Trayectoria profesional del vicepresidente..."
    },
    {
        id: "presidente-tribunal-constitucional",
        cargo: "Presidente del Tribunal Constitucional",
        nombre: "Excmo. Sr. Don Antonio Nzambi Nlonga",
        foto: "/images/cspj/antonio.webp",
        descripcion: "Breve descripción de las funciones principales de los Vocales del Consejo Superior del Poder Judicial...",
        funciones: [
            "Participar en las deliberaciones del Consejo",
            "Integrar comisiones específicas",
            "Votar en las decisiones del órgano"
        ],
        trayectoria: "Trayectoria profesional del vocal..."
    },
    {
        id: "ministro-justicia-derechos-humanos",
        cargo: "Ministro de Justicial y Derechos Humanos",
        nombre: "Excmo. Sr. Don Reginaldo Biyogo Mba Ndong Anguesomo",
        foto: "/images/cspj/reginaldo.webp",
        descripcion: "Responsable de la secretaría del Consejo Superior...",
        funciones: [
            "Levantar actas de las sesiones",
            "Custodiar la documentación",
            "Coordinar la agenda del Consejo"
        ],
        trayectoria: "Trayectoria profesional del secretario general..."
    },
    {
        id: "fiscal-general",
        cargo: "Fiscal General de la República",
        nombre: "Excmo. Sr. Don Anatolio Nzang Nguema",
        foto: "/images/cspj/anatolio.webp",
        descripcion: "Asiste al Secretario General en sus funciones...",
        funciones: [
            "Apoyar al Secretario General",
            "Gestionar documentación administrativa",
            "Coordinar con las diferentes áreas"
        ],
        trayectoria: "Trayectoria profesional del secretario adjunto..."
    },
    {
        id: "decano-abogados",
        cargo: "Decano del Ilustre Colegio de Abogados",
        nombre: "Excmo. Sr. Don Juan Olo Mba Nzeng",
        foto: "/images/cspj/juan.webp",
        descripcion: "Breve descripción de las funciones principales de los Vocales del Consejo Superior del Poder Judicial...",
        funciones: [
            "Participar en las deliberaciones del Consejo",
            "Integrar comisiones específicas",
            "Votar en las decisiones del órgano"
        ],
        trayectoria: "Trayectoria profesional del vocal..."
    },
    {
        id: "secretario-general",
        cargo: "Secretario General del Consejo Superior del Poder Judicial",
        nombre: "Ilmo. Sr. Don Andres Ondo Nze Nzang",
        foto: "/images/cspj/andres.webp",
        descripcion: "Responsable de la gestión de archivos y sistemas informáticos...",
        funciones: [
            "Gestionar el archivo general del Poder Judicial",
            "Supervisar sistemas informáticos",
            "Implementar nuevas tecnologías"
        ],
        trayectoria: "Trayectoria profesional de la directora..."
    },
    {
        id: "secretarios-general-adjunto",
        cargo: "Secretario General Adjunto del Consejo Superior del Poder Judicial",
        nombre: "Ilmo. Sr. Don Pascual Ekoro Ekoro Avomo",
        foto: "/images/cspj/pascual.webp",
        descripcion: "Responsable de la gestión económica y financiera...",
        funciones: [
            "Gestionar el presupuesto del Poder Judicial",
            "Supervisar gastos e inversiones",
            "Coordinar con el Ministerio de Hacienda"
        ],
        trayectoria: "Trayectoria profesional del director..."
    },
    {
        id: "director-juridicos",
        cargo: "Director General de Asuntos Economicos y Financieros del Consejo Superior del Poder Judicial",
        nombre: "Nombre del Director General",
        foto: "/images/cspj/agustin.webp",
        descripcion: "Responsable de los asuntos jurídicos del Consejo...",
        funciones: [
            "Asesorar en materia jurídica",
            "Elaborar propuestas normativas",
            "Coordinar aspectos legales"
        ],
        trayectoria: "Trayectoria profesional del director..."
    },
        {
        id: "magistrado3",
        cargo: "Directora General de Archivo, Documentación e Informatica",
        nombre: "Ilma. Sra. Doña Liliosa Ada Biko Bengobesam",
        foto: "/images/csj/liliosa.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    }
    // {
    //     id: "director-juridicos",
    //     cargo: "Director General de Asuntos Jurídicos",
    //     nombre: "Nombre del Director General",
    //     foto: "/images/consejo/director-juridicos.jpg",
    //     descripcion: "Responsable de los asuntos jurídicos del Consejo...",
    //     funciones: [
    //         "Asesorar en materia jurídica",
    //         "Elaborar propuestas normativas",
    //         "Coordinar aspectos legales"
    //     ],
    //     trayectoria: "Trayectoria profesional del director..."
    // }
];

export const corteSuprema = [
    {
        id: "presidente-corte-suprema",
        cargo: "Presidente de la Corte Suprema de Justicia",
        nombre: "Excmo. Sr. Don Francisco EVUY NGUEMA MIKUE",
        foto: "/images/cspj/francisco.webp",
        descripcion: "Máxima autoridad judicial de la Corte Suprema de Justicia...",
        funciones: [
            "Presidir las sesiones de la Corte Suprema",
            "Representar al máximo órgano judicial",
            "Coordinar con el Consejo Superior del Poder Judicial",
            "Velar por la correcta aplicación de la ley"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Corte Suprema..."
    },
    {
        id: "magistrado1",
        cargo: "Magistrado de la Corte Suprema de Justicia",
        nombre: " Excmo. Sr. Don Serafin Ondo Maye",
        foto: "/images/csj/serafin.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado2",
        cargo: "Magistrado de la Corte Suprema de Justicia",
        nombre: "Excmo. Sr. Don. Eliseo Mangue Nvo Oyana",
        foto: "/images/csj/eliseo.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    // Puedes añadir más magistrados según sea necesario
    {
        id: "magistrado3",
        cargo: "Magistrado de la Corte Suprema de Justicia",
        nombre: "Excmo. Sr. Don Hilario Ballovera Casaña",
        foto: "/images/csj/hilario.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado3",
        cargo: "Magistrada de la Corte Suprema de Justicia",
        nombre: "Excma. Sra. Doña Rosa Maricarmen Obama Obono",
        foto: "/images/csj/rosa.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado4",
        cargo: "Magistrada de la Corte Suprema de Justicia",
        nombre: "Excma. Sra. Doña Clara Feliciddad Bonkanka Tabares",
        foto: "/images/csj/clara.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado5",
        cargo: "Magistrado de la Corte Suprema de Justicia",
        nombre: "Excmo. Sr. Don Victoriano Obiang Abogo",
        foto: "/images/csj/victoriano.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado6",
        cargo: "Magistrado de la Corte Suprema de Justicia",
        nombre: "Excmo. Sr. Don Candido Nkua Nkogo",
        foto: "/images/csj/candido.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    // {
    //     id: "magistrado7",
    //     cargo: "Magistrado Juez de la Corte Suprema",
    //     nombre: "Excmo. Sra. Doña Rosa Maricarmen OBAMA OBONO",
    //     foto: "/images/corte-suprema/magistrado3.jpg",
    //     descripcion: "Magistrado de la Corte Suprema de Justicia...",
    //     funciones: [
    //         "Conocer y resolver los recursos de casación",
    //         "Participar en las deliberaciones de la Corte",
    //         "Emitir votos en las decisiones judiciales"
    //     ],
    //     trayectoria: "Trayectoria profesional del magistrado..."
    // },
    {
        id: "magistrado3",
        cargo: "Secretario de lo Civil de la Corte Suprema de Justicia",
        nombre: "Ilmo. Sr. Don Carlos Sorizo Buesahe",
        foto: "/images/csj/carlos.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado3",
        cargo: "Secretario de la sala de lo social y contenciosos administrativos",
        nombre: "Ilmo. sr. don Jose Abeso Ondo,",
        foto: "/images/csj/jose.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado3",
        cargo: "Secretaria de lo penal de la corte suprema de justicia",
        nombre: "Ilma. sra. doña Maria Carmen Mbeze Nguema Mofuman ",
        foto: "/images/csj/maria.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    // {
    //     id: "magistrado3",
    //     cargo: "Magistrado Juez de la Corte Suprema",
    //     nombre: "Excmo. Sra. Doña Rosa Maricarmen OBAMA OBONO",
    //     foto: "/images/corte-suprema/magistrado3.jpg",
    //     descripcion: "Magistrado de la Corte Suprema de Justicia...",
    //     funciones: [
    //         "Conocer y resolver los recursos de casación",
    //         "Participar en las deliberaciones de la Corte",
    //         "Emitir votos en las decisiones judiciales"
    //     ],
    //     trayectoria: "Trayectoria profesional del magistrado..."
    // },
    {
        id: "magistrado3",
        cargo: "Director General de Prensa de la Corte Suprema de Justicia",
        nombre: " Ilmo. Sr. Don Hilario Mitogo Miko Mikue,",
        foto: "/images/csj/mitogo.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
    {
        id: "magistrado3",
        cargo: "Director General de Protocolo y Ceremonias de la Corte Suprema de Justicia",
        nombre: "Ilmo. Sr. Don Martin Mba Angue Bilogo",
        foto: "/images/csj/martin.webp",
        descripcion: "Magistrado de la Corte Suprema de Justicia...",
        funciones: [
            "Conocer y resolver los recursos de casación",
            "Participar en las deliberaciones de la Corte",
            "Emitir votos en las decisiones judiciales"
        ],
        trayectoria: "Trayectoria profesional del magistrado..."
    },
];

// Orden personalizado de los cargos
export const ordenCargos = [
    "Presidente del Consejo Superior del Poder Judicial",
    "Vicepresidente del Consejo Superior del Poder Judicial",
    "Vocal del Consejo Superior del Poder Judicial",
    "Secretario General",
    "Secretario Adjunto",
    "Directora General de Archivos e Informática",
    "Director General de Asuntos Económicos",
    "Director General de Asuntos Jurídicos"
];

export const ordenCargosCorteSuprema = [
    "Presidente de la Corte Suprema de Justicia",
    "Magistrado Juez de la Corte Suprema",
    "Magistrado Juez de la Corte Suprema",
    "Secretario de la Corte Suprema",
    "Magistrado Juez de la Corte Suprema"
];
// Orden de cargos para las Audiencias
export const ordenCargosAudiencias = [
    "Presidente de la Audiencia",
    "Magistrado",
    "Juez de Instrucción 1",
    "Juez de Instrucción 2",
    "Juez de Instrucción 3",
    "Juez de Primera Instancia 1",
    "Juez de Primera Instancia 2",
    "Secretario de la Audiencia"
];

// Audiencia de Malabo
export const audienciaMalabo = [
    {
        id: "presidente-audiencia-malabo",
        cargo: "Presidente de la Audiencia",
        audiencia: "Malabo",
        nombre: "Nombre del Presidente",
        foto: "/images/audiencias/malabo/presidente.jpg",
        descripcion: "Presidente de la Audiencia Provincial de Malabo, responsable de la administración de justicia en la capital...",
        funciones: [
            "Presidir la Audiencia Provincial de Malabo",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Malabo..."
    },
    {
        id: "magistrado-malabo",
        cargo: "Magistrado",
        audiencia: "Malabo",
        nombre: "Nombre del Magistrado",
        foto: "/images/audiencias/malabo/presidente.jpg",
        descripcion: "Magistrado de la Audiencia Provincial de Malabo, responsable de la administración de justicia en la capital...",
        funciones: [
            "Presidir la Audiencia Provincial de Malabo",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Malabo..."
    },
    {
        id: "juez-instruccion-1-malabo",
        cargo: "Juez de Instrucción 1",
        audiencia: "Malabo",
        nombre: "Nombre del Juez de Instrucción 1",
        foto: "/images/audiencias/malabo/juez-instruccion-1.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Malabo...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-2-malabo",
        cargo: "Juez de Instrucción 2",
        audiencia: "Malabo",
        nombre: "Nombre del Juez de Instrucción 2",
        foto: "/images/audiencias/malabo/juez-instruccion-2.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Malabo...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-3-malabo",
        cargo: "Juez de Instrucción 3",
        audiencia: "Malabo",
        nombre: "Nombre del Juez de Instrucción 3",
        foto: "/images/audiencias/malabo/juez-instruccion-3.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Malabo...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-primera-instancia-1-malabo",
        cargo: "Juez de Primera Instancia 1",
        audiencia: "Malabo",
        nombre: "Nombre del Juez de Primera Instancia 1",
        foto: "/images/audiencias/malabo/juez-primera-instancia-1.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Malabo...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    },
    {
        id: "juez-primera-instancia-2-malabo",
        cargo: "Juez de Primera Instancia 2",
        audiencia: "Malabo",
        nombre: "Nombre del Juez de Primera Instancia 2",
        foto: "/images/audiencias/malabo/juez-primera-instancia-2.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Malabo...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    }
];

// Audiencia de Bata
export const audienciaBata = [
    {
        id: "presidente-audiencia-bata",
        cargo: "Presidente de la Audiencia",
        audiencia: "Bata",
        nombre: "Nombre del Presidente",
        foto: "/images/audiencias/bata/presidente.jpg",
        descripcion: "Presidente de la Audiencia Provincial de Bata, responsable de la administración de justicia en la región continental...",
        funciones: [
            "Presidir la Audiencia Provincial de Bata",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Bata..."
    },
    {
        id: "magistrado-bata",
        cargo: "Magistrado",
        audiencia: "Bata",
        nombre: "Nombre del Magistrado de Bata",
        foto: "/images/audiencias/malabo/presidente.jpg",
        descripcion: "Magistrado de la Audiencia Provincial de Malabo, responsable de la administración de justicia en la capital...",
        funciones: [
            "Presidir la Audiencia Provincial de Malabo",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Malabo..."
    },
    {
        id: "juez-instruccion-1-bata",
        cargo: "Juez de Instrucción 1",
        audiencia: "Bata",
        nombre: "Nombre del Juez de Instrucción 1",
        foto: "/images/audiencias/bata/juez-instruccion-1.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Bata...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-2-bata",
        cargo: "Juez de Instrucción 2",
        audiencia: "Bata",
        nombre: "Nombre del Juez de Instrucción 2",
        foto: "/images/audiencias/bata/juez-instruccion-2.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Bata...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-3-bata",
        cargo: "Juez de Instrucción 3",
        audiencia: "Bata",
        nombre: "Nombre del Juez de Instrucción 3",
        foto: "/images/audiencias/bata/juez-instruccion-3.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Bata...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-primera-instancia-1-bata",
        cargo: "Juez de Primera Instancia 1",
        audiencia: "Bata",
        nombre: "Nombre del Juez de Primera Instancia 1",
        foto: "/images/audiencias/bata/juez-primera-instancia-1.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Bata...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    },
    {
        id: "juez-primera-instancia-2-bata",
        cargo: "Juez de Primera Instancia 2",
        audiencia: "Bata",
        nombre: "Nombre del Juez de Primera Instancia 2",
        foto: "/images/audiencias/bata/juez-primera-instancia-2.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Bata...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    }
];

// Audiencia de Centro-Sur
export const audienciaCentroSur = [
    {
        id: "presidente-audiencia-centro-sur",
        cargo: "Presidente de la Audiencia",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Presidente",
        foto: "/images/audiencias/centro-sur/presidente.jpg",
        descripcion: "Presidente de la Audiencia Provincial de Centro-Sur...",
        funciones: [
            "Presidir la Audiencia Provincial de Centro-Sur",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Centro-Sur..."
    },
    {
        id: "magistrado-centro-sur",
        cargo: "Magistrado",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Magistrado de Centro Sur",
        foto: "/images/audiencias/malabo/presidente.jpg",
        descripcion: "Magistrado de la Audiencia Provincial de Malabo, responsable de la administración de justicia en la capital...",
        funciones: [
            "Presidir la Audiencia Provincial de Malabo",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Malabo..."
    },
    {
        id: "juez-instruccion-1-centro-sur",
        cargo: "Juez de Instrucción 1",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Juez de Instrucción 1",
        foto: "/images/audiencias/centro-sur/juez-instruccion-1.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Centro-Sur...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-2-centro-sur",
        cargo: "Juez de Instrucción 2",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Juez de Instrucción 2",
        foto: "/images/audiencias/centro-sur/juez-instruccion-2.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Centro-Sur...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-3-centro-sur",
        cargo: "Juez de Instrucción 3",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Juez de Instrucción 3",
        foto: "/images/audiencias/centro-sur/juez-instruccion-3.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Centro-Sur...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-primera-instancia-1-centro-sur",
        cargo: "Juez de Primera Instancia 1",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Juez de Primera Instancia 1",
        foto: "/images/audiencias/centro-sur/juez-primera-instancia-1.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Centro-Sur...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    },
    {
        id: "juez-primera-instancia-2-centro-sur",
        cargo: "Juez de Primera Instancia 2",
        audiencia: "Centro-Sur",
        nombre: "Nombre del Juez de Primera Instancia 2",
        foto: "/images/audiencias/centro-sur/juez-primera-instancia-2.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Centro-Sur...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    }
];

// Audiencia de Kie-Ntem
export const audienciaKieNtem = [
    {
        id: "presidente-audiencia-kie-ntem",
        cargo: "Presidente de la Audiencia",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Presidente",
        foto: "/images/audiencias/kie-ntem/presidente.jpg",
        descripcion: "Presidente de la Audiencia Provincial de Kie-Ntem...",
        funciones: [
            "Presidir la Audiencia Provincial de Kie-Ntem",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Kie-Ntem..."
    },
    {
        id: "magistrado-KieNtem",
        cargo: "Magistrado",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Magistrado de Kie-Ntem",
        foto: "/images/audiencias/malabo/presidente.jpg",
        descripcion: "Magistrado de la Audiencia Provincial de Malabo, responsable de la administración de justicia en la capital...",
        funciones: [
            "Presidir la Audiencia Provincial de Malabo",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Malabo..."
    },
    {
        id: "juez-instruccion-1-kie-ntem",
        cargo: "Juez de Instrucción 1",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Juez de Instrucción 1",
        foto: "/images/audiencias/kie-ntem/juez-instruccion-1.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Kie-Ntem...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-2-kie-ntem",
        cargo: "Juez de Instrucción 2",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Juez de Instrucción 2",
        foto: "/images/audiencias/kie-ntem/juez-instruccion-2.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Kie-Ntem...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-3-kie-ntem",
        cargo: "Juez de Instrucción 3",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Juez de Instrucción 3",
        foto: "/images/audiencias/kie-ntem/juez-instruccion-3.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Kie-Ntem...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-primera-instancia-1-kie-ntem",
        cargo: "Juez de Primera Instancia 1",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Juez de Primera Instancia 1",
        foto: "/images/audiencias/kie-ntem/juez-primera-instancia-1.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Kie-Ntem...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    },
    {
        id: "juez-primera-instancia-2-kie-ntem",
        cargo: "Juez de Primera Instancia 2",
        audiencia: "Kie-Ntem",
        nombre: "Nombre del Juez de Primera Instancia 2",
        foto: "/images/audiencias/kie-ntem/juez-primera-instancia-2.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Kie-Ntem...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    }
];

// Audiencia de Wele-Nzas
export const audienciaWeleNzas = [
    {
        id: "presidente-audiencia-wele-nzas",
        cargo: "Presidente de la Audiencia",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Presidente",
        foto: "/images/audiencias/wele-nzas/presidente.jpg",
        descripcion: "Presidente de la Audiencia Provincial de Wele-Nzas...",
        funciones: [
            "Presidir la Audiencia Provincial de Wele-Nzas",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Wele-Nzas..."
    },
    {
        id: "magistrado-wele-nzas",
        cargo: "Magistrado",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Magistrado de Wele Nzas",
        foto: "/images/audiencias/malabo/presidente.jpg",
        descripcion: "Magistrado de la Audiencia Provincial de Malabo, responsable de la administración de justicia en la capital...",
        funciones: [
            "Presidir la Audiencia Provincial de Malabo",
            "Coordinar las actividades judiciales de la provincia",
            "Supervisar el trabajo de los jueces de instrucción y primera instancia",
            "Representar la audiencia ante otras instituciones"
        ],
        trayectoria: "Trayectoria profesional del presidente de la Audiencia de Malabo..."
    },
    {
        id: "juez-instruccion-1-wele-nzas",
        cargo: "Juez de Instrucción 1",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Juez de Instrucción 1",
        foto: "/images/audiencias/wele-nzas/juez-instruccion-1.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Wele-Nzas...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-2-wele-nzas",
        cargo: "Juez de Instrucción 2",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Juez de Instrucción 2",
        foto: "/images/audiencias/wele-nzas/juez-instruccion-2.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Wele-Nzas...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-instruccion-3-wele-nzas",
        cargo: "Juez de Instrucción 3",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Juez de Instrucción 3",
        foto: "/images/audiencias/wele-nzas/juez-instruccion-3.jpg",
        descripcion: "Juez de Instrucción de la Audiencia Provincial de Wele-Nzas...",
        funciones: [
            "Dirigir la investigación de delitos",
            "Dictar medidas cautelares",
            "Decidir sobre la apertura del juicio oral",
            "Garantizar los derechos del investigado"
        ],
        trayectoria: "Trayectoria profesional del juez de instrucción..."
    },
    {
        id: "juez-primera-instancia-1-wele-nzas",
        cargo: "Juez de Primera Instancia 1",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Juez de Primera Instancia 1",
        foto: "/images/audiencias/wele-nzas/juez-primera-instancia-1.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Wele-Nzas...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    },
    {
        id: "juez-primera-instancia-2-wele-nzas",
        cargo: "Juez de Primera Instancia 2",
        audiencia: "Wele-Nzas",
        nombre: "Nombre del Juez de Primera Instancia 2",
        foto: "/images/audiencias/wele-nzas/juez-primera-instancia-2.jpg",
        descripcion: "Juez de Primera Instancia de la Audiencia Provincial de Wele-Nzas...",
        funciones: [
            "Conocer asuntos civiles y mercantiles",
            "Resolver conflictos de primera instancia",
            "Dictar sentencias en su ámbito de competencia",
            "Garantizar el debido proceso"
        ],
        trayectoria: "Trayectoria profesional del juez de primera instancia..."
    }
];

// Array combinado de todas las audiencias para la página principal
export const todasLasAudiencias = [
    ...audienciaMalabo,
    ...audienciaBata,
    ...audienciaCentroSur,
    ...audienciaKieNtem,
    ...audienciaWeleNzas
];